...
- **Inert primary CTA**: same shape as the accent Save button (`rounded-md bg-accent px-4 py-2 text-sm font-medium text-accent-foreground`) plus `opacity-60 cursor-not-allowed` and `disabled`. Use for Feature-gated actions that must stay visible but non-submitting (Generate Resume until Feature 08).
